#!/usr/bin/env node
// Fetch the public Ausbildungssuche `X-API-Key` from the authoritative upstream
// source.
//
...
// outside the program, and hand it in via `--api-key` or the
// AUSBILDUNGSSUCHE_API_KEY environment variable. Typical CI usage:
//
//   AUSBILDUNGSSUCHE_API_KEY="$(node scripts/fetch-api-key.mjs)" npm test
//
// Source: the bundesAPI README is what the rendered ausbildungssuche.api.bund.dev
// docs are generated from, and it carries the key in plain text (the docs site
// is a JS SPA, so the README is the machine-readable source). Scraped with a
// simple regex on purpose — no HTML parser, no dependencies.
...
export async function fetchApiKey() {
  const res = await fetch(SOURCE_URL, {
    headers: { "User-Agent": "ausbildungssuche-cli fetch-api-key" },
  });
  if (!res.ok) {
